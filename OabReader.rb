  ...
  def _scalar(type)
    case type
      ...
      when :AnsiString, :UnicodeString
        ...
        string.force_encoding(MapiPropertyDataType.UnicodeString ? 'UTF-8' : 'ASCII')
        return string
    end
...
if __FILE__ == $0
  ...
  reader = OabReader.new(oab)
  puts reader.header.inspect
  reader.records.each{|record|
    record.to_h.keys.sort.each{|k|
      v = record[k]
      v = v[0] if v.length == 1
      v = v.inspect if k == :AddressBookObjectGuid
      puts "#{k}: #{v}"
    }
    puts
  }
